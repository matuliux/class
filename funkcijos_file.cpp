...
void failas() {
    ...
    try {
        if (fd.is_open()) {
            ...
            sort_mok(list);
            vertinimas(list, mokiniu_sk, pazangumas);
            file_isvedimas(list, mokiniu_sk, isvedimas1, isvedimas2);
...
void file_isvedimas(vector<Studentas> list, int sk, double& isvedimas1, double& isvedimas2) {
    ...
    if (file == "0") {
        ...
        for (int i = 0; i < sk; i++) {
            if (list[i].grupe == 0) {
                fu << setw(20) << list[i].vardas << setw(20) << list[i].pavarde << setw(17) << std::fixed << std::setprecision(2) << list[i].rez
                    << list[i].med << "\n";
            }
        }
...
void vertinimas(vector<Studentas>& list, int sk, double& rusiavimas) {

    auto start = hrClock::now();
    for (long long int i = 0; i < sk; i++) {
        if (list[i].rez >= 5.0) {
            list[i].grupe = 1;
        }
    }
...
void sort_mok(vector<Studentas>& list) {
    ...
    do {
        cout << "Pagal ka norite isrusiuoti faila? 1 - Vardus 2 - Pavardes 0 - Nerusiuoti "; cin >> rusiavimas;
        if (rusiavimas == "1" || rusiavimas == "2" || rusiavimas == "0")r_i = 1;
        else cout << "Klaidingas ivedimas" << "\n";
    } while (r_i == 0);
