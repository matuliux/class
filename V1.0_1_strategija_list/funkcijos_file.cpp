...
void failas() {
    list <Studentas> pirm, protingi, vargsiukai;
    Studentas nw;
    ...
    try {
        if (fd.is_open()) {
            ...
            sort_mok(pirm);
            vertinimas(pirm, protingi, vargsiukai, pazangumas);
...
void vertinimas(list<Studentas> pirm, list<Studentas>& protingi, list<Studentas>& vargsiukai, double& pazangumas) {
    auto start = hrClock::now();
    for (const auto& kint : pirm) { //(auto &kint:list)
...
void sort_mok(list<Studentas>& pirm) {

    pirm.sort(compare_last);
